        ...
        <form onSubmit={(event) => sendLoginForm(event)} className='flex flex-col items-start justify-center'>
            {isError === true &&
                <div className="w-[435px] h-[38px] flex justify-start items-center gap-2 rounded-[6px] bg-[#FFF0F3] mb-[24px]">
                    ...
                    <p className="text-[14px] font-medium">Your email or password is incorrect</p>
                </div>
            }

            <label htmlFor="email" className="mb-1.5 font-medium text-[14px] text-[#0D0D12]">Email</label>
            <div className={`w-[435px] h-12 flex items-center bg-gray-100 border ${isError === true ? 'border-[#EC778D] shadow-[0px_0px_3px_#F2D7DF]': email.length > 0? 'border-[#429EFF] shadow-[0px_0px_3px_#285DF2]':   'border-gray-400'} rounded-full px-4 py-2 mb-6`}>
            ...
            <div className={`w-[435px] h-12 flex items-center bg-gray-100 border ${isError === true ? 'border-[#EC778D] shadow-[0px_0px_3px_#F2D7DF]':password.length > 0? 'border-[#429EFF] shadow-[0px_0px_3px_#285DF2]':   'border-gray-400'} rounded-full px-4 py-2 mb-6`}>
                <FiLock size={24} className="text-gray-400 mr-2" />
                <input
                    type='text'
                    id='password'
                    value={visiblePassword? password: '*'.repeat(password.length)}
            ...
            <div className='flex flex-col justify-center gap-4 mb-6'>
                <button className={`w-[435px] h-[54px] rounded-[50px] text-[#818898] text-[16px] font-medium ${email.length > 8 && password.length > 8? 'text-white bg-[#429EFF] cursor-pointer': 'bg-[#ECEFF3] cursor-not-allowed'}`} disabled={!(email.length > 8 && password.length > 8)}>Login</button>
                <div className='flex flex-row items-center justify-between'>
                    ...
                    <Link to={'/'} className="text-[14px] text-[#6F6F6F] font-medium">Forgot Password ?</Link>
                </div>
            </div>
                {isSuccess === true && showModal === false && <SuccessModal title={'Login Successful'} message={'Let\'s get started and take your customer support dashboard to the next level!'} onClose={() => {setShowModal(!showModal)}}/>}
