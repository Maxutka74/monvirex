        ...
        <>
            <form onSubmit={(event) => sendResetPasswordForm(event)} className='flex flex-col items-start justify-center'>
                ...
                <div className="flex flex-col items-center justify-center  mb-6">
                    <button className={`w-[435px] h-[44px] rounded-[50px] text-[#818898] text-[16px] font-medium ${email.trim().length > 0? 'text-white bg-[#429EFF] cursor-pointer': 'bg-[#ECEFF3] cursor-not-allowed'} mb-6`} disabled={!(email.trim().length > 0)}>Continue</button>
                    <Link to="/" className="w-[85px] h-[40px] flex flex-row items-center justify-center gap-3"><GoArrowLeft />Back</Link>
                </div>
            </form>
        </>
    )
