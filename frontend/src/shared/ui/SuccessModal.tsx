...
const SuccessModal = ({ title, message, link, buttonName, onClose }: SuccessModalProps) => {
    return (
        <div className="fixed inset-0 bg-black/50 grid place-items-center min-h-screen z-1">
            <div className="w-[500px] h-[360px] flex flex-col items-center rounded-[20px] bg-[#FFFFFF]">
                ...
                <Link to={link}><button type="button" className="w-[452px] h-[50px] rounded-[50px] text-white bg-[#429EFF] mt-5 cursor-pointer" onClick={() => localStorage.clear()}>{buttonName}</button></Link>
            </div>
        </div>
    )
}
