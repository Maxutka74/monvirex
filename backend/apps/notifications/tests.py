...
class NotificationServiceTest(TestCase):
    ...
    def test_create_notification(self):
        notification = NotificationService.create_notification(
            ...
            message='Your deposit of 100 USDT has been successfully added to your balance'
        )
    ...
    def test_get_notifications_returns_only_user_notifications(self):
        ...
        NotificationService.create_notification(
            ...
            message=f'Your withdrawal of 10 USD has been successfully processed.'
        )
        NotificationService.create_notification(
            user=self.user_two,
            notification_type='withdraw',
            title='Withdraw successful',
            message=f'Your withdrawal of 1000 USD has been successfully processed.'
        )
    ...
    def test_mark_as_read_marks_notification(self):
        ...
        notification_before = NotificationService.mark_as_read(user=self.user_one, notification_id=notification_after.id)
    ...
    def test_mark_as_read_foreign_notification_returns_none(self):
        ...
        notification_none = NotificationService.mark_as_read(user=self.user_two, notification_id=notification_after.id)
    ...
    def test_mark_all_as_read_marks_only_current_user_notifications(self):
        ...
        notifications_all_read = NotificationService.get_notifications(user=self.user_one)

        self.assertEqual(notifications_all_read.first().is_read, True)

        notifications_not_read = NotificationService.get_notifications(user=self.user_two)
    ...
    def test_get_unread_count(self):
        ...
        NotificationService.mark_as_read(user=self.user_one, notification_id=notification.id)
...
class NotificationApiTest(APITestCase):
    ...
    def test_mark_as_read_foreign_notification_api_returns_404(self):
        ...
        response_read = self.client.patch(f'/api/notifications/{notification_id.id}/read/')
...
class NotificationIntegrationTest(TestCase):
    def setUp(self):
        ...
        Asset.objects.create(symbol='BTCUSDT', name='BTC', current_price=50000, price_change_24h=0, volume_24h=0)
        Asset.objects.create(symbol='ETHUSDT', name='ETH', current_price=10000, price_change_24h=0, volume_24h=0)
    ...
    def test_withdraw_creates_notification(self):
        ...
        self.assertEqual(notification.first().message, 'Your withdrawal of 100 USD has been successfully processed.')
        self.assertEqual(notification.first().user, self.user_one)
    ...
    def test_buy_creates_notification(self):
        ...
        self.assertEqual(notification.first().message, 'You bought 0.1 BTCUSDT for 5000 USD.')
        self.assertEqual(notification.first().user, self.user_one)

    def test_sell_creates_notification(self):
        TradeService.buy(user=self.user_one, symbol='BTCUSDT', amount_usdt=5000)
        TradeService.sell(user=self.user_one, symbol='BTCUSDT', amount_crypto=Decimal(0.001))

        notification = NotificationService.get_notifications(user=self.user_one)

        self.assertEqual(notification.count(), 2)
        self.assertEqual(notification.first().notification_type, 'sell')
        self.assertEqual(notification.first().message, 'You sold 0.001 BTCUSDT for 50.0 USD.')
        self.assertEqual(notification.first().user, self.user_one)
    ...
    def test_exchange_creates_notification(self):
        ...
        TradeService.exchange(user=self.user_one, from_asset='BTCUSDT', to_asset='ETHUSDT', amount_crypto=Decimal(0.001))

        notification = NotificationService.get_notifications(user=self.user_one)

        self.assertEqual(notification.count(), 3)
        self.assertEqual(notification.first().notification_type, 'exchange')
        self.assertEqual(notification.first().message, 'You exchanged 0.001 BTCUSDT to 0.005 ETHUSDT.')
        self.assertEqual(notification.first().user, self.user_one)
